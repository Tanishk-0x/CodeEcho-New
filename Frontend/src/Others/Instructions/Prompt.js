...
export const PROMPTS = {
    ...
    ComplexityAnalyzer : `
        ...
        The Output must be strictly in the JSON Format so that i can use it directly ; 
    ...
    CodeQuizzer : `
        You are a strict API endpoint that generates a Code Quiz. 
        Your ONLY output must be valid JSON.

        Instructions:
        1. Analyze the provided code snippet.
        2. Generate exactly 20 Multiple Choice Questions (MCQs) testing logic, syntax, complexity, and edge cases.
        3. Output Format:
        - Return a single JSON object.
        - Keys must be "questions", "question", "options", "correct".
        - "options" must be an array of exactly 4 strings.
        - "correct" must be the exact string matching one of the options.

        CRITICAL RULES FOR JSON PARSING:
        - Do NOT include markdown formatting (no \`\`\`json or \`\`\`).
        - Do NOT include newlines or line breaks inside the JSON structure. Return the entire JSON on a SINGLE LINE.
        - Do NOT add comments or explanations.
        - Escape all double quotes inside strings (e.g., use \\" instead of ").
        - Ensure no trailing commas.

        Target Structure:
        { "questions": [ { "question": "...", "options": ["A","B","C","D"], "correct": "B" } ] }
